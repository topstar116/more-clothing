...
<!-- タイトル・メタディスクリプション -->
@section('title', 'ユーザー一覧 | モアクロ')
@section('description', 'ユーザー一覧')
    ...
    <div id="modal-stop" uk-modal>
        <div class="uk-modal-dialog">
            ...
            <div class="uk-modal-header">
                <h2 class="uk-modal-title">本当に削除しますか？</h2>
            </div>
            <form method="POST" action="{{ route('admin.edit.user.stop') }}">
                @csrf
                <input id="target_id" name="user_id" type="hidden" value="">
    ...
    <div class="admin-body">
        <table class="uk-table uk-table-responsive uk-table-divider">
            ...
            <tbody>
                ...
                <tr>
                    <td style="white-space: nowrap"><a class="u-text--link" href="{{ route('admin.show.user', $user->id) }}">{{ $user->last_name }} {{ $user->first_name }}<br>{{ $user->last_name_kana }} {{ $user->first_name_kana }}</a></td>
                    <td>{{ $user->phone_number }}</td>
                    <td>{{ $user->email }}</td>
                    <td>{{ $user->memo }}</td>
                    <td style="white-space: nowrap">
                        <button class="c-button c-button--bgPink u-mr10" type="button" uk-toggle="target: #modal-stop" onclick="event.preventDefault();document.getElementById('target_id').value = {{ $user->id }}">削除</button>
